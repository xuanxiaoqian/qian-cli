...
function createRootRouter(moduleName) {
  ...
  if (_data.router.isPageDir) {
    if (import_fs.default.existsSync(import_path.default.join(renderPageUrl, moduleName, vueName))) {
      console.log(
        (0, import_kolorist.red)(`${import_path.default.join(renderPageUrl, moduleName, vueName)} \u5B58\u5728\uFF01\uFF01\uFF01`)
      );
      process.exit(0);
    }
  } else {
  ...
  `;
  let sass = import_common.currentPackageJson["devDependencies"]["sass"] ?? import_common.currentPackageJson["dependencies"]["sass"] ?? false;
...
function createChildren(moduleName) {
  ...
  if (_data.router.isPageDir) {
    if (!import_fs.default.existsSync(
      import_path.default.join(import_path.default.join(process.cwd(), _data.router.pagePath), fatherName)
    )) {
      console.log(
        (0, import_kolorist.red)(
          `\u627E\u4E0D\u5230 ${import_path.default.join(
            import_path.default.join(process.cwd(), _data.router.pagePath),
            fatherName
          )}`
        )
      );
    ...
    if (import_fs.default.existsSync(
      import_path.default.join(
        import_path.default.join(process.cwd(), _data.router.pagePath, sonName + ".vue")
      )
    )) {
      console.log(
        (0, import_kolorist.red)(
